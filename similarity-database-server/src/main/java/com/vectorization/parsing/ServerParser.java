...
public class ServerParser extends Parser<ServerCommand> {
    ...
    private Vector vector(String name) {
        ...
        while (getLookAhead().type.equals(SSType.NUMBER) || getLookAhead().type.equals(SSType.NEGNUMBER)) {
            if (getLookAhead().type.equals(SSType.NUMBER)) {
                vals.add(real());
            } else if (getLookAhead().type.equals(SSType.NEGNUMBER)) {
                vals.add(negreal());
            }
